...
namespace Microsoft.Dafny.Compilers {
  public class KotlinCodeGenerator : SinglePassCodeGenerator {
    ...
    protected override ConcreteSyntaxTree EmitIngredients(ConcreteSyntaxTree wr, string ingredients, int L, string tupleTypeArgs, ForallStmt s, SingleAssignStmt s0, Expression rhs) {
      var wStmts = wr.Fork();
      var wrVarInit = wr;
      wrVarInit.Write($"ArrayList<{DafnyTupleClass(L)}<{tupleTypeArgs}>> {ingredients} = ");
    ...
    protected override void DeclareSubsetType(SubsetTypeDecl sst, ConcreteSyntaxTree wr) {
      ...
      if (sst.WitnessKind == SubsetTypeDecl.WKind.Compiled) {
        ...
        cw.StaticMemberWriter.Write($"{TypeParameters(sst.TypeArgs, " ")}{typeName} defaultValue(");
    ...
    protected class ClassWriter : IClassWriter {
      ...
      public ClassWriter(KotlinCodeGenerator codeGenerator, ConcreteSyntaxTree instanceMemberWriter, ConcreteSyntaxTree ctorBodyWriter, ConcreteSyntaxTree staticMemberWriter = null) {
    ...
    protected ConcreteSyntaxTree CreateGetter(string name, Type resultType, IOrigin tok, bool isStatic,
      ...
      wr.Write("fun {0}(): {1}", name, TypeName(resultType, wr, tok));
      if (createBody) {
    ...
    public ConcreteSyntaxTree /*?*/ CreateGetterSetter(string name, Type resultType, IOrigin tok,
      ...
      wr.Write($"fun set_{name}({TypeName(resultType, wr, tok)} value)");
      if (createBody) {
    ...
    protected ConcreteSyntaxTree CreateMethod(MethodOrConstructor m, List<TypeArgumentInstantiation> typeArgs, bool createBody, ConcreteSyntaxTree wr, bool forBodyInheritance, bool lookasideBody) {
      ...
      // Kotlin syntax: fun methodName(params): ReturnType (no static/void keywords)
      wr.Write("{0}", !createBody && !(m.EnclosingClass is TraitDecl) ? "abstract " : "");
      wr.Write("fun ");
    ...
    protected ConcreteSyntaxTree/*?*/ CreateFunction(string name, List<TypeArgumentInstantiation> typeArgs,
      ...
      wr.Write("{0}", !createBody && !(member.EnclosingClass is TraitDecl) ? "abstract " : "");
      wr.Write("fun ");
      ...
      wr.Write("): {0}", TypeName(resultType, wr, tok));
      if (!createBody) {
        wr.WriteLine("");  // No semicolon in Kotlin
        return null; // We do not want to write a function body, so instead of returning a BTW, we return null.
      } else {
    ...
    protected void DeclareField(string name, bool isStatic, bool isConst, Type type, IOrigin tok, string rhs, ClassWriter cw) {
      ...
      if (isStatic) {
        ...
        cw.InstanceMemberWriter.WriteLine("var {1}: {0}", TypeName(type, cw.InstanceMemberWriter, tok), name);
    ...
    protected override IClassWriter CreateClass(string moduleName, bool isExtern, string /*?*/ fullPrintName,
      ...
      w.Write($"{abstractness}class {javaName}{TypeParameters(typeParameters)}");
      string sep;
    ...
    private void EmitTypeDescriptorMethod([CanBeNull] TopLevelDecl enclosingTypeDecl, List<TypeParameter> typeParams, string targetTypeName,
      ...
      if (typeParams.Count == 0) {
        ...
        wr.WriteLine($"private val {DafnyTypeDescriptor}<{targetTypeName}> _TYPE = {typeDescriptorExpr};");
        typeDescriptorExpr = "_TYPE";
      }
      wr.Write($"{TypeParameters(typeParams, " ")}{DafnyTypeDescriptor}<{targetTypeName}> {TypeMethodName}(");
    ...
    private string CastIfSmallNativeType(NativeType nt) {
      switch (AsKotlinNativeType(nt)) {
        case KotlinNativeType.Byte: return "(byte) ";
        case KotlinNativeType.Short: return "(short) ";
        case KotlinNativeType.Long: return "(long) ";
        default: return "";
      }
    }
    ...
    protected override void EmitLiteralExpr(ConcreteSyntaxTree wr, LiteralExpr e) {
      if (e is StaticReceiverExpr) {
        ...
        if (i.IsZero) {
          ...
          wr.Write($"java.math.BigInteger(\"{i}\")");
        }
      } else if (e.Value is BaseTypes.BigDec n) {
        if (0 <= n.Exponent) {
          wr.Write($"{DafnyBigRationalClass}(java.math.BigInteger(\"{n.Mantissa}");
          ...
          wr.Write($"java.math.BigInteger(\"{n.Mantissa}\")");
          wr.Write(", java.math.BigInteger(\"1");
    ...
    protected string GetNativeDefault(NativeType nt) {
      switch (AsKotlinNativeType(nt)) {
        case KotlinNativeType.Byte: return "(byte) 0";
        case KotlinNativeType.Short: return "(short) 0";
    ...
    protected override void GetNativeInfo(NativeType.Selection sel, out string name, out string literalSuffix,
      ...
      switch (AsKotlinNativeType(sel)) {
        case KotlinNativeType.Byte: name = "byte"; needsCastAfterArithmetic = true; break;
        case KotlinNativeType.Short: name = "short"; needsCastAfterArithmetic = true; break;
        case KotlinNativeType.Int: name = "int"; break;
        case KotlinNativeType.Long: name = "long"; literalSuffix = "L"; break;
        default:
          Contract.Assert(false);  // unexpected native type
          throw new Cce.UnreachableException();  // to please the compiler
      }
    ...
    private string GetBoxedNativeTypeName(NativeType nt) {
      switch (AsKotlinNativeType(nt)) {
        case KotlinNativeType.Byte: return "Byte";
        case KotlinNativeType.Short: return "Short";
        case KotlinNativeType.Int: return "Int";
        case KotlinNativeType.Long: return "Long";
        default:
          Contract.Assert(false);  // unexpected native type
          throw new Cce.UnreachableException();  // to please the compiler
    ...
    protected override void DeclareLocalVar(string name, Type /*?*/ type, IOrigin /*?*/ tok, bool leaveRoomForRhs,
      ...
      wr.Write("var {0}: {1}", name, type != null ? TypeName(type, wr, tok) : "Any");
      if (leaveRoomForRhs) {
    ...
    protected override void EmitMapDisplay(MapType mt, IOrigin tok, List<MapDisplayEntry> elements, bool inLetExprBody,
      ...
      wr.Write($"({tuple2}<{BoxedTypeName(mt.Domain, wr, tok)}, {BoxedTypeName(mt.Range, wr, tok)}>[]){tuple2}[]{{");
      string sep = " ";
    ...
    IClassWriter CompileDatatypeBase(DatatypeDecl dt, ConcreteSyntaxTree wr) {
      ...
      EmitSuppression(wr);
      wr.Write("class {0}{1}", dt.IsRecordType ? "" : " abstract", DtT_protected);
      ...
      if (dt.TypeArgs.Count == 0) {
        wr.Write($"private val theDefault: {simplifiedTypeName} = ");
        wDefault = wr.Fork();
        wr.WriteLine("");  // No semicolon in Kotlin
        var w = wr.NewBlock($"fun Default(): {simplifiedTypeName}");
        ...
        wr.Write($"{sep}{typeParameters}): {simplifiedTypeName}");
        var w = wr.NewBlock("");
        w.Write("return ");
        wDefault = w.Fork();
        w.WriteLine("");
      ...
      if (dt.HasFinitePossibleValues) {
        ...
        var w = wr.NewNamedBlock($"fun AllSingletonConstructors(): ArrayList<{DtT_protected}>");
        string arraylist = "singleton_iterator";
        w.WriteLine($"val {arraylist} = ArrayList<{DtT_protected}>();");
    ...
    void CompileDatatypeConstructors(DatatypeDecl dt, ConcreteSyntaxTree wrx) {
      ...
      if (dt is CoDatatypeDecl) {
        ...
        w.WriteLine($"interface Computer {{ fun run(): {dt.GetCompileName(Options)} }}");
        w.WriteLine("var c: Computer?");
        w.WriteLine($"var d: {dt.GetCompileName(Options)}{typeParams}?");
        ...
        var wCtorBody = w.NewBlock($"constructor({wCtorParams}{sep}c: Computer)");
    ...
    void DatatypeFieldsAndConstructor(DatatypeCtor ctor, int constructorIndex, ConcreteSyntaxTree wr) {
      ...
      foreach (Formal arg in ctor.Formals) {
        if (!arg.IsGhost) {
          wr.WriteLine($"val {FieldName(arg, i)}: {TypeName(arg.Type, wr, arg.Origin)}");
          i++;
        }
      }
      ...
      if (dt is CoDatatypeDecl) {
        string typeParams = TypeParameters(dt.TypeArgs);
        wr.WriteLine($"{dt.GetCompileName(Options)}{typeParams} Get() {{ return this; }}");
      }
      ...
      wr.WriteLine();
      {
        ...
        w.WriteLine("if (this.javaClass != other.javaClass) return false");
        string typeParams = TypeParameters(dt.TypeArgs);
      ...
      wr.WriteLine();
      {
        ...
        if (dt is TupleTypeDecl && ctor.Formals.Count == 0) {
          ...
          w.WriteLine($"StringBuilder {tempVar} = StringBuilder();");
    ...
    private ConcreteSyntaxTree EmitToString(ConcreteSyntaxTree wr, Type type) {
      ...
      if (AsNativeType(type) != null && AsNativeType(type).LowerBound >= 0) {
        ...
        if (type.NormalizeToAncestorType().IsStringType) {
          ...
          wr.Write($"{{ _s: {DafnySeqClass}<*> -> if (_s.elementType().defaultValue()::class.java == Character::class.java) _s.verbatimString() else _s.toString() }}(");
          argumentWriter = wr.Fork();
          wr.Write(")");
        } else {
          wr.Write("(");
    ...
    protected override string TypeDescriptor(Type type, ConcreteSyntaxTree wr, IOrigin tok) {
      ...
      if (type is BoolType) {
        ...
        if (at.Dims > 1) {
          ...
          switch (AsKotlinNativeType(elType)) {
            case KotlinNativeType.Byte: return $"{DafnyTypeDescriptor}.BYTE_ARRAY";
            case KotlinNativeType.Short: return $"{DafnyTypeDescriptor}.SHORT_ARRAY";
            case KotlinNativeType.Int: return $"{DafnyTypeDescriptor}.INT_ARRAY";
            case KotlinNativeType.Long: return $"{DafnyTypeDescriptor}.LONG_ARRAY";
            default:
              Contract.Assert(false);
              throw new Cce.UnreachableException();
          }
        ...
        return $"(({DafnyTypeDescriptor}<{typeName}>)" +
               "(Any)" +
               $"{DafnyTypeDescriptor}.reference({typeNameSansTypeParameters}::class.java))";
    ...
    protected override void EmitSetBuilder_New(ConcreteSyntaxTree wr, SetComprehension e, string collectionName) {
      wr.WriteLine($"ArrayList<{BoxedTypeName(e.Type.NormalizeToAncestorType().AsSetType.Arg, wr, e.Origin)}> {collectionName} = ArrayList();");
    }
    ...
    protected override void EmitMapBuilder_New(ConcreteSyntaxTree wr, MapComprehension e, string collectionName) {
      ...
      wr.WriteLine($"java.util.HashMap<{BoxedTypeName(domType, wr, e.Origin)}, {BoxedTypeName(ranType, wr, e.Origin)}> {collectionName} = java.util.HashMap<>();");
    }
    ...
    private string HelperClass(NativeType nt) {
      return AsKotlinNativeType(nt) == KotlinNativeType.Long ? "java.lang.Long" : "java.lang.Integer";
    }
    ...
    protected override ConcreteSyntaxTree DeclareLocalVar(string name, Type type, IOrigin tok, ConcreteSyntaxTree wr) {
      wr.Write("var {0}: {1} = ", name, type != null ? TypeName(type, wr, tok, false, false, null) : "Any");
      var w = wr.Fork();
      wr.WriteLine("");
      return w;
    }
    ...
    protected override IClassWriter CreateTrait(string name, bool isExtern, List<TypeParameter> typeParameters /*?*/,
      ...
      w.WriteLine();
      EmitSuppression(w); //TODO: Fix implementations so they do not need this suppression
      w.Write($"class _Companion_{name}{typeParamString}");
      var staticMemberWriter = w.NewBlock("");
      var ctorBodyWriter = staticMemberWriter.NewBlock($"_Companion_{name}()");
    ...
    protected override IClassWriter DeclareNewtype(NewtypeDecl nt, ConcreteSyntaxTree wr) {
      ...
      if (nt.NativeType != null) {
        ...
        var wEnum = w.NewNamedBlock($"ArrayList<{nativeType}> IntegerRange(java.math.BigInteger lo, java.math.BigInteger hi)");
        wEnum.WriteLine($"ArrayList<{nativeType}> arr = ArrayList();");
      ...
      if (nt.WitnessKind == SubsetTypeDecl.WKind.Compiled) {
        ...
        if (nt.NativeType == null) {
          ...
          w.Write("{0} Witness = ((java.lang.Number) (", nativeType);
          w.Append(witness);
    ...
    void GenerateIsMethod(RedirectingTypeDecl declWithConstraints, ConcreteSyntaxTree wr) {
      ...
      if (declWithConstraints.ConstraintIsCompilable) {
        ...
        wr.Write($"{TypeParameters(declWithConstraints.TypeArgs, " ")}boolean {IsMethodName}(");
    ...
    void DeclareBoxedNewtype(NewtypeDecl nt, ConcreteSyntaxTree wr) {
      ...
      wr.WriteLine($"{targetTypeName} _value;");

      // constructor:
      // public NewType(TargetRepresentation value) {
      //   _value = value;
      // }
      wr.NewNamedBlock($"{IdName(nt)}({targetTypeName} value)")
      ...
      var wBody = wr.NewNamedBlock("String toString()");
      wBody.Write("return ");
    ...
    protected override void EmitNewArray(Type elementType, IOrigin tok, List<string> dimensions,
      ...
      if (dimensions.Count > 1) {
        wr.Write($"{DafnyMultiArrayClass(dimensions.Count)}<>({TypeDescriptor(elementType, wr, tok)}, ");
    ...
    protected override ConcreteSyntaxTree EmitDowncast(Type from, Type to, IOrigin tok, ConcreteSyntaxTree wr) {
      ...
      if (from != null && to != null && from.IsTraitType && to.AsNewtype != null) {
        ...
        wr.Write($"(({TypeName(to, wr, tok)})(Any)(");
        w = wr.Fork();
        wr.Write("))");
      }
      return w;
    ...
    protected override ConcreteSyntaxTree CreateStaticMain(IClassWriter cw, string argsParameterName) {
      ...
      return wr.NewBlock($"void __Main(dafny.DafnySequence<dafny.DafnySequence<? : {CharTypeName(true)}>> {argsParameterName})");
    }
